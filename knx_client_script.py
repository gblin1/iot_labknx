...
def send_knx_request(
    ...
    while conn_resp.status != 0:
        # Disconnect request to gateway
        disc_req = knxnet.create_frame(
        knxnet.ServiceTypeDescriptor.DISCONNECT_REQUEST,
        channel_id_retr,
        control_endpoint
        )
        logger.debug('>>> DISCONNECT_REQ:\n{}'.format(disc_req))
        sock.sendto(disc_req.frame, (gateway_ip, gateway_port))

        # Disconnect response from gateway
        logger.debug('Waiting for disconnect response (DISCONNECT_RESPONSE -> disc_resp')
        data_recv, addr = sock.recvfrom(buf_size)
        disc_resp = knxnet.decode_frame(data_recv)
        logger.info('tunnelling acknowledge status: {}'.format(disc_resp.status))
        # must check if status equals 0x00
    ...
    channel_id_retr = conn_resp.channel_id
    print('Status of connection: {} '.format(conn_resp.status))
    print('Channel_id retrieved from connection: {}'.format(channel_id_retr))



    # Connection state request
    conn_st_req = knxnet.create_frame(
        knxnet.ServiceTypeDescriptor.CONNECTION_STATE_REQUEST,
        channel_id_retr,  # channel_id retrieved from the conn_resp
        control_endpoint
        )
    logger.debug('>>> CONNECTION_STATE_REQUEST:\n{}'.format(conn_st_req))
    sock.sendto(conn_st_req.frame, (gateway_ip, gateway_port))

    # Connection state response
    logger.debug('Waiting for connection state response (CONNECTION_STATE_RESPONSE -> conne_st_resp)')
    data_recv, addr = sock.recvfrom(buf_size)
    conn_st_resp = knxnet.decode_frame(data_recv)
    logger.info('state response status: {}'.format(conn_st_resp.status)) # receives only a status as a response
    # must check if status equals 0x00

    print('connection state response status: {}'.format(conn_st_resp.status))
    
    groupAddress,payload = build_target_command(target, args)

    if conn_st_resp.status == 0:
    ...
    # Tunneling acknowledge
    logger.debug('Waiting for tunnelling acknowledge (TUNNELLING_ACK -> tunn_ack')
    data_recv, addr = sock.recvfrom(buf_size)
    tunn_ack = knxnet.decode_frame(data_recv)
    logger.info('tunnelling acknowledge status: {}'.format(tunn_ack.status))
    # must check if status equals 0x00
    print('tuunneling ack status: {}'.format(tunn_ack.status))
    


    # Tunnelling request from gateway
    logger.debug('Waiting for tunnelling request from Gateway (TUNNELLING_REQ -> tunn_req_interface')
    data_recv, addr = sock.recvfrom(buf_size)
    tunn_req_interface = knxnet.decode_frame(data_recv)
    logger.info('tunnelling request: {}'.format(tunn_req_interface.data_service))
    print('received tunneling rquest from gateway, channel_id: {}'.format(tunn_req_interface.channel_id))
    print('received tunneling rquest from gateway, data: {}'.format(tunn_req_interface.data))
    print('received tunneling rquest from gateway, data_size: {}'.format(tunn_req_interface.data_size))
    print('received tunneling rquest from gateway, acpi: {}'.format(tunn_req_interface.acpi))
    print('received tunneling rquest from gateway, sequence_counter: {}'.format(tunn_req_interface.sequence_counter))

    # Tunnelling acknowledge to gateway
    tunn_ack_interface = knxnet.create_frame(
        knxnet.ServiceTypeDescriptor.TUNNELLING_ACK,
        channel_id_retr,
        0,
        0
        )
    logger.debug('>>> TUNNELLING_ACKNOWLEDGE:\n{}'.format(tunn_ack_interface))
    sock.sendto(tunn_ack_interface.frame, (gateway_ip, gateway_port))
    ...
    data_recv, addr = sock.recvfrom(buf_size)
    disc_resp = knxnet.decode_frame(data_recv)
    logger.info('disc_resp.channel_id: {}'.format(disc_resp.channel_id))

    # print('disconnect response status from gateway: {}'.format(disc_resp.status))
    print('disconnect response channel_id from gateway: {}'.format(disc_resp.channel_id))
    # logger.info('tunnelling acknowledge status: {}'.format(disc_resp.status))
    # must check if status equals 0x00
